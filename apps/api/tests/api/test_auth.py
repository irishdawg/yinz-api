...
def test_garbage_bearer_token_is_rejected():
    ...
    client = TestClient(create_app(repository=InMemoryGameRepository()), headers={"x-gotiate-gateway-key": settings.gotiate_gateway_secret or ""})
...
def test_missing_gateway_secret_gets_disguised_as_not_found():
    ...
    client = TestClient(create_app(repository=InMemoryGameRepository()))  # no x-gotiate-gateway-key at all
...
def test_wrong_gateway_secret_gets_disguised_as_not_found():
    client = TestClient(create_app(repository=InMemoryGameRepository()), headers={"x-gotiate-gateway-key": "wrong-secret"})
    response = client.get("/games/some-id")
    assert response.status_code == 404
    assert response.json() == {"detail": "Not Found"}
...
def test_gateway_rejection_matches_a_real_404_byte_for_byte():
    client = TestClient(create_app(repository=InMemoryGameRepository()))
    rejected = client.get("/games/some-id")
    real_404 = TestClient(
        create_app(repository=InMemoryGameRepository()), headers={"x-gotiate-gateway-key": settings.gotiate_gateway_secret or ""}
